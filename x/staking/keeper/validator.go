...
func (k Keeper) AddValidatorTokensAndShares(ctx sdk.Context, validator types.Validator,
	...
	k.DeleteValidatorByTicket(ctx, validator)
	...
	k.SetValidatorByTicket(ctx, validator)
...
func (k Keeper) RemoveValidatorTokens(ctx sdk.Context,
	...
	k.Logger(ctx).Debug(
		fmt.Sprintf("RemoveValidatorTokens %s", sdk.AccAddress(validator.OperatorAddress)),
	)
...
// returns an iterator for the current validator ticket store
func (k Keeper) ValidatorTicketStoreIterator(ctx sdk.Context) sdk.Iterator {
	store := ctx.KVStore(k.storeKey)
	return sdk.KVStorePrefixIterator(store, types.ValidatorsByTicketKey)
}
